...
type Props = RootStackScreenProps<'ChatRequests'>;

export const ChatRequestsScreen: React.FC<Props> = ({ navigation }) => {
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: T.bg }]} edges={['top']}>
      ...
      <View style={[styles.topBar, { borderBottomColor: T.borderSubtle }]}>
        ...
        <Text style={[styles.title, { color: T.text }]}>Chat Requests</Text>
        <View style={{ width: 24 }} /> {/* Balance spacer */}
      </View>

      {/* Body / Empty State */}
      <View style={styles.center}>
        <Ionicons name="mail-unread-outline" size={64} color={T.textMuted} />
        <Text style={[styles.emptyTitle, { color: T.text }]}>No pending requests</Text>
